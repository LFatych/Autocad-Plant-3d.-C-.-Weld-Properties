...
WeldPropUtils/WeldPropUtils/
  ...
  WeldAutoUpdate.cs         auto-update of new welds: properties and/or numbers (events, see Roadmap 3); PluginApp.cs starts it
  WeldNumbering.cs          weld numbers: NumberAll (full renumber), WeldNumberIndex (numbers for new welds), ParseSize
...
| `WeldPropMapping` | Opens the SPDS **Weld Property Mapping** window: drag part properties (read from Project Setup) onto weld properties, add any weld class property per side ("+ Add weld property"), weld properties missing in Project Setup are shown red, manage profiles, dark/light theme. **Automation & numbering** card holds all switches: fill new welds, number new welds, first weld numbers. **Save** writes the settings file; **Save and number all welds** = Save + SetWeldNumber; **Save and update all welds** also runs the mapping on the drawing. |
...
| `SetWeldNumber` | Runs `SetWeldProp`, then groups welds of one type that have the same OD, wall thickness and material on both ports (sizes compared as numbers). It numbers the groups sorted by size, largest first, starting from 11 for butt welds, 51 for taps and 71 for socket welds (`numbering` in the settings). Writes the result to `WeldNumber` with `dlm.SetProperties`. |

Automatic numbering (`numbering.auto`): a new weld gets the number of an existing group with the same key
(`WeldNumbering.GroupKey`), otherwise the highest number of its type + 1. Existing numbers never change. The index of
used numbers is read once per drawing (all welds) and replaced after SetWeldNumber; numbers edited by hand are not seen
until the drawing is reopened.
...
- Content: `schemaVersion`, `activeProfile`, `autoUpdate` (new welds filled automatically; default off), `numbering` (start numbers for
  Buttweld/Tap/Socketweld, `auto` = number new welds; default off), and `profiles[]`, each with `name`, `mirrorSides` and `mappings[]` of `{target, side (1|2), source}`.
...
- Roadmap (agreed with the user):
  ...
  3. ✅ (v1, untested in Plant 3D) Auto-update of new welds (`WeldAutoUpdate`): `Database.ObjectAppended` only collects the
     ...
     without a new connector) are not re-filled yet. Also numbers new welds when `numbering.auto` is on (user request).
...
3. ✅ Fixed: sizes are compared as numbers via `WeldNumbering.ParseSize` (invariant culture, accepts `114,3` and units).
...
7. ✅ Fixed: `WeldNumber` is written with `dlm.SetProperties` like the other properties.
